...
function initChatLoadBox(){
  const el=document.getElementById("lcInfoText"); if(!el) return;
  const texts=[];
  for(const [cat,apis] of Object.entries(dat)) for(const [name,api] of Object.entries(apis)){
    texts.push(name+".js");
  }
  texts.push("Tip: klik card endpoint untuk coba langsung");
  texts.push(Object.keys(dat).length+" kategori tersedia");
  texts.push(Object.values(dat).reduce((a,b)=>a+Object.keys(b).length,0)+" total endpoints");
  let i=0;
  function rotate(){
    if(!el) return;
    el.style.opacity="0";
...
function lcSend(){
  ...
  lcAppend("lc-user",msg); input.value="";
  const typing=document.getElementById("lcTyping");
  typing.style.display="flex"; lcScroll();
  setTimeout(()=>{
    typing.style.display="none";
    lcAppend("lc-bot",staticReply(msg));
    lcScroll();
  },600+Math.random()*400);
}
function lcAppend(role,text){
  const msgs=document.getElementById("lcMsgs");
  const div=document.createElement("div"); div.className="lc-msg "+role;
  div.innerHTML="<div class=\"lc-bubble\">"+text+"</div><div class=\"lc-time\">"+nowTime()+"</div>";
  msgs.insertBefore(div,document.getElementById("lcTyping"));
}
function lcScroll(){ const m=document.getElementById("lcMsgs"); if(m) setTimeout(()=>m.scrollTo({top:m.scrollHeight,behavior:"smooth"}),50); }



document.getElementById("lcInput").addEventListener("keydown",e=>{ if(e.key==="Enter"&&!e.shiftKey){ e.preventDefault(); lcSend(); } });
