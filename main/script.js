...
function initSearch(){
  const wrap = document.querySelector(".sec-head");
  if(document.getElementById("searchInput")) return;
  const input = document.createElement("input");
  input.id = "searchInput";
  input.className = "search-input";
  input.placeholder = "Search endpoint...";
...
function buildUrlHtml(api){
  ...
  let html = `<span class="ep-base">${base()}/</span><span class="ep-name" style="color:#fff">${api.path}</span>`;
...
function buildStatsData(){
  ...
  const statusTable = allApis.slice(0,8).map(ep=>({ name:ep.name, cat:ep.cat, hits:Math.round(Math.random()*3000+100), status:ep.status }));
  return { requests:Math.round(7000+Math.random()*3000), uptime:99.2+Math.random()*0.7, avgMs:Math.round(130+Math.random()*60), activeEp:allApis.filter(a=>a.status).length, topEndpoints, perKategori, responseTime, statusTable };
}
...
function buildTwLines(){
  ...
  if(!twLines.length) twLines = [base()+"/api/info/api"];
}
function typeTick(){
